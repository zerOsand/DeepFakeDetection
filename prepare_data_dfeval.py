...
def copy_images(dest_dir, max_images=10):
    # Load metadata
    df = pd.read_csv(os.path.join("datasets", "Deepfake-Eval-2024", "image-metadata-publish.csv"))
    df = df[df["Finetuning Set"] == "Test"]
    df = df[~df["Filename"].str.contains("webp", na=False)]  # Exclude "webp" files
    df["name"] = df["Ground Truth"].apply(lambda x: "F_" if x == "Fake" else "R_") + df["Filename"]
    ...
    for file_name in filenames:
        ...
        if os.path.isfile(file_path) and Path(file_path).suffix.lower() in img_extensions:
            ...
            print(f"Skipped: {actual_file_name} (File not found or unsupported extension)")
